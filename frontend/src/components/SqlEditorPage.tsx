...
async function ensureSqlJs(indexUrl: string): Promise<SqlJsStatic> {
  ...
  sqlJsLoader = (async () => {
    ...
    // const init = (window as unknown as { initSqlJs?: any }).initSqlJs;
...
function quoteIdent(name: string): string {
  // SQLite identifiers can be double-quoted. Escape internal quotes by doubling.
...
function inferSqlType(values: Array<Cell>): "INTEGER" | "REAL" | "TEXT" {
  for (const v of values) {
    ...
    // Everything else -> TEXT (including ISO datetime strings)
    return "TEXT";
  }
  return "TEXT";
}
...
export default function SqlEditorPage({ dataset_id, onBack }: Props) {
  ...
  const datasetTitle = useMemo(() => {
    if (!dataset_id) return "SQL Editor";
    if (!info) return "SQL Editor";
    return `SQL Editor • ${info.display_name}`;
  }, [dataset_id, info]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setSqlReady(true);
        setRunNote(
          "Loaded the preview rows into an in-browser SQLite table named: data"
        );
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        if (!cancelled) {
          const m = message || "Failed to load dataset";
          // Heuristic: if this looks like a runtime load failure, show it separately.
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        {runtimeErr && (
          <p className="text-gray-700">
            Failed to load SQL runtime in the browser: {runtimeErr}
          </p>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {!sqlReady && (
                  <span className="text-sm text-gray-600">Preparing SQL…</span>
                )}
              </div>
            </div>
